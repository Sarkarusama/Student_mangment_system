...
do {
    ...
    if (action === chalk.cyan("Enroll a student")) {
        ...
        let trimedStudentName = studentName.trim().toLowerCase();
        let studentNamesCheck = students.map((obj) => obj.name);
        if (studentNamesCheck.includes(trimedStudentName) === false) {
            if (trimedStudentName !== "") {
                baseID++;
                studentID = "STID" + baseID;
                console.log(chalk.green("\n\t✨ Your account has been created."));
                console.log(chalk.bold.green(`\t\tWelcome, ${trimedStudentName}!\n`));
                ...
                if (courseConfirmation === true) {
                    let student = new Student(trimedStudentName, studentID, [course], courseFees);
